...
class BaseTUDLDataset(Dataset):
    def __init__(self, option, partition, cls_nms):
        self.option = option
        self.db_nm = option.db_nm
        self.test_mode = option.test_mode
        self.partition = partition
        self.is_debug_mode = option.is_debug_mode
    ...
    def get_annos_info(self):
        ...
        for cls_nm in self.cls_nms:
            ...
            gen_depths_bboxes_path = os.path.join(self.gen_db_dir, "%s_%s_%s_depths_bboxes_tiny%d.pth" % (
                                                            self.db_nm, self.partition, cls_nm, int(self.is_debug_mode)))
    ...
    def get_src_pcd(self, src_idx):
        ...
        src_pcd = regularize_pcd(src_pcd, self.n_points // 2, test_mode=self.test_mode)  # [3, N]
        return src_pcd, src_gt_bbox

    def get_model_pcd(self, cls_nm):
        model_pcd = regularize_pcd(self.models_info[cls_nm]["model_pcd"], self.n_points, test_mode=self.test_mode)  # [3, M]
        model_gt_bbox = self.models_info[cls_nm]["model_bbox"]
        return model_pcd, model_gt_bbox

class TUDL_DB_Train(BaseTUDLDataset):
    def __init__(self, option, partition, cls_nms):
        super(TUDL_DB_Train, self).__init__(option, partition, cls_nms)
        self.cls_nms = cls_nms
    ...
    def gen_reg_sample(self, res, src_pcd, model_pcd, src_gt_bbox, model_gt_bbox, sample_idx):
        ...
        if self.option.normalization_enabled:
            res["src_pcd_normal"] = cal_normal(X.T, radius=self.option.radius, max_nn=30).astype(np.float32)
            res["model_pcd_normal"] = cal_normal(Y.T, radius=self.option.radius, max_nn=30).astype(np.float32)
...
class TUDL_DB_Test(BaseTUDLDataset):
    def __init__(self, option, partition, cls_nms):
        super(TUDL_DB_Test, self).__init__(option, partition, cls_nms)
        self.cls_nms = cls_nms
    ...
    def getitem(self, idx):
        ...
        if self.option.normalization_enabled:
            res["src_pcd_normal"] = cal_normal(trans_src_pcd.T, radius=self.option.radius, max_nn=30).astype(np.float32)
            res["model_pcd_normal"] = cal_normal(model_pcd.T, radius=self.option.radius, max_nn=30).astype(np.float32)
